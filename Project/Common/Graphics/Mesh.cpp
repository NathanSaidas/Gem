...
namespace Gem
{
    ...
    bool Mesh::Upload()
    {
        ...
        UInt32 count = m_Positions.GetCount();
        if (count != m_Normals.GetCount() ||
            count != m_TexCoords.GetCount() ||
            count != m_Colors.GetCount()
            )
        {
            Debug::ErrorFormat("Gem", nullptr, "There is a different number of attributes.");
            return false;
        }
        ...
        {
            //Positions
            attributes[i].position[0] = m_Positions[i].x;
            attributes[i].position[1] = m_Positions[i].y;
            attributes[i].position[2] = m_Positions[i].z;

            //Normals
            attributes[i].normal[0] = m_Normals[i].x;
            attributes[i].normal[1] = m_Normals[i].y;
            attributes[i].normal[2] = m_Normals[i].z;

            //Tex Coords
            attributes[i].texCoord[0] = m_TexCoords[i].x;
            attributes[i].texCoord[1] = m_TexCoords[i].y;

            //Colors
            attributes[i].color[0] = m_Colors[i].r;
            attributes[i].color[1] = m_Colors[i].g;
            attributes[i].color[2] = m_Colors[i].b;
            attributes[i].color[3] = m_Colors[i].a;
        }

        glGenBuffers(1, &m_VBO);
        glBindBuffer((GLenum)BufferTarget::Array, m_VBO);
        ...
        glBindBuffer((GLenum)BufferTarget::ElementArray, m_IBO);
        glBufferData((GLenum)BufferTarget::ElementArray, sizeof(GLshort)* m_Indices.GetCount(), m_Indices.GetElements(), (GLenum)BufferMode::StaticDraw);
